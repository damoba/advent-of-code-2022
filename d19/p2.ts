...
const parseBlueprints = (data: string): Costs[] => {
  ...
  let i = 0;
  while (i < MAX_BLUEPRINTS && i < lines.length) {
    const line = lines[i];
...
// Calculates whether if in theory we only built geode robots from now on,
// we would have more geodes than the current max and thus be worth it.
// --  credit: https://nickymeuleman.netlify.app/garden/aoc2022-day19
const robotIsWorthIt = (pathStep: PathStep, maxGeodes: { val: number }) => {
  const { mats, numRobots, time } = pathStep;
  const timeLeft = TIME_END - time.mins;

  // ((t-1) * t) / 2 is the triangular number of the time left
  //   i.e. the number of geodes collected increasing by one every minute
  //   since one geode robot is built every minute and will collect on the next
  // g + (t * r) is the geodes we already have plus the geodes we will collect
  //   with the robots we already have
  return (
    ((timeLeft - 1) * timeLeft) / 2 + mats.geode + timeLeft * numRobots.geode >
    maxGeodes.val
  );
};
...
const respondToBuildResult = (
  ...
  if (isPathEnd(buildResult)) {
    ...
    buildResult.nextPathStep &&
    robotIsWorthIt(buildResult.nextPathStep, maxGeodes)
  ) {
...
const calcMaxGeodes = (blueprint: Costs) => {
  ...
  while (pathSteps.length) {
    ...
    maxGeodes.val = Math.max(maxGeodes.val, pathStep.mats.geode);
...
const solve = (data: string) => {
  ...
  return geodesPerBlueprint.reduce((ga, gb) => ga * gb, 1);
};

const example = await Deno.readTextFile("./example.txt");
const input = await Deno.readTextFile("./input.txt");
Deno.test("Test and Solve", () => {
  assertEquals(solve(example), 3472);
