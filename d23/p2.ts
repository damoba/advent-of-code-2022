...
const solve = (data: string) => {
  ...
  let rounds = 0;
  let settledElves = 0;
  while (settledElves < elves.vals.length) {
    settledElves = performRound(map, elves, rounds % PROPOSAL_ORDER.length);
    rounds++;
  }
